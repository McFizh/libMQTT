...
class Client {
    ...
    /** @var bool $verifyPeerName	verify CA peer name */
    ...
    function connect($clean = true) {
        ...
        if (ord($string{0}) == 0x20 && $string{3} == chr(0)) {
            ...
            $msg = sprintf("Connection failed! Error: 0x%02x 0x%02x", ord($string{0}), ord($string{3}));
    ...
    /**
     * Sets CAfile peer name verification
     *
     * @param bool $value
     */
    function setVerifyPeerName($value) {
        $this->verifyPeerName = $value;
    }
